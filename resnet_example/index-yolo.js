async function predict() {
  var be = document.getElementById("backend").value
  console.log(be)
  // Create an ONNX inference session with WebGL backend.
  const session = new onnx.InferenceSession({ backendHint: be });
  ...
  const imageData = await imageLoader.getImageData('./car.png');
 ...
 */
function printMatches(data, process_time) {
  let outputClasses = [];
  ...
  const results = [];
  results.push(`Execution Time: ${process_time} milliseconds`)
